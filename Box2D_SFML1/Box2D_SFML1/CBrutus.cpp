...
CBrutus::CBrutus(sf::RenderWindow* _renderWindow, AudioManager* _audioManager)
{
	m_AudioManager = _audioManager;
	m_RenderWindow = _renderWindow;

	m_SpriteTexture = new sf::Texture();
	m_CubeType = CUBEMONTYPE::BRUTUS;

	Start();
}

CBrutus::CBrutus(sf::RenderWindow* _renderWindow, b2World* _world, b2Body& _playerBody)
{
	m_RenderWindow = _renderWindow;
	m_World = _world;
...
void CBrutus::Start()
{
	m_Name = "Brutus";
	CreateShape();
	LoadSpriteTexture(LoadTexture(m_SpriteTexture, "Cubemon/Plant_Elemental.png", false), m_Shape);
